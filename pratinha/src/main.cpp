...
#if defined(NDEBUG)
# error "Pratinha cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7ad87a943062bf1bb3dc9499b0c2266aae03d2a00b9ea5bf3e03eb1226651aeb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pratinha: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pratinha Signed Message:\n";
...
{
    // Pratinha: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Pratinha: 210k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 7 * 24 * 60 * 60; // Pratinha: 7 days
static const int64 nTargetSpacing = 2 * 60; // Pratinha: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pratinha: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pratinha: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd40b702cba0d3f72f7aff9821527e7b2d62046a7892a1bf65aa9d91b006f2379");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Reuters 01/Jul/2022 Russian missile strikes near Ukraine's Odesa kill 21";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 21 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0445663abccf704f3196dc635a0cbb47910732d793ddfdf87529fe03d75309ad899e5f50266a56604f79c1a799c1aae0d1197113ce9ce99280bbdbb6e478383a61") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1656721645;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086309664;

        if (fTestNet)
        {
            block.nTime    = 1656721629;
            block.nNonce   = 386521210;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7119e1465d3cac2f13e63caae130f08521fcad5e857246b5aeb1bb7aea3a7a3a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc9, 0xb7, 0xdd }; // Pratinha: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // transactions:
            if (!fSortedByFee &&
                ((nBlockSize + nTxSize >= nBlockPrioritySize) || (dPriority < COIN * 5040 / 250)))
            {
                fSortedByFee = true;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PratinhaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PratinhaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PratinhaMiner(CWallet *pwallet)
{
    printf("PratinhaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pratinha-miner");
    ...
    try { loop {
        ...
        printf("Running PratinhaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PratinhaMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PratinhaMiner, pwallet));
}
